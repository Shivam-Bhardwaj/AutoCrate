  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start space-x-3">
        <input
          type="checkbox"
          checked={markings.appliedMaterialsLogo}
          onChange={() => toggleMarking('logo')}
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
            {logoDims && (
              <>
                <div>Size: {formatDimensions(logoDims)}</div>
                <div>P/N: {logoDims.partNumber}</div>
                <div>Position: Upper left corner of each panel (4 per crate)</div>
              </>
            )}
          </div>
        </div>
