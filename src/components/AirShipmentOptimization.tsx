...
export default function AirShipmentOptimization() {
  ...
  const calculateWeightSavings = useCallback(() => {
  ...
  }, [configuration.airShipment, weightBreakdown?.total]);

  // Calculate volume reduction
  const calculateVolumeReduction = useCallback(() => {
    if (!configuration.airShipment?.enabled || !configuration.airShipment?.chamfer.enabled) {
      return;
    }
  ...
  }, [configuration, weightSavings, volumeReduction]);
